/**
 * Admin API Routes — Agile Business Admin Panel
...
module.exports = function createAdminRouter(pool, hooks = {}) {
    ...
    async function getSetting(key) {
        const rows = await q('SELECT setting_value FROM settings WHERE setting_key = ?', [key]);
        return rows.length ? rows[0].setting_value : '';
    }
    async function setSetting(key, value) {
        await q('INSERT INTO settings (setting_key, setting_value) VALUES (?, ?) ON DUPLICATE KEY UPDATE setting_value = ?', [key, value, value]);
    }
    ...
    async function ensureUniqueSlug(tableName, wantedSlug, currentId, maxLen) {
        ...
        while (true) {
            const rows = currentId
                ? await q(`SELECT id FROM ${tableName} WHERE slug = ? AND id <> ? LIMIT 1`, [candidate, currentId])
                : await q(`SELECT id FROM ${tableName} WHERE slug = ? LIMIT 1`, [candidate]);
    ...
    async function ensureAdminTables() {
        try {
            ...
            await pool.execute(`CREATE TABLE IF NOT EXISTS tasks (
                id INT AUTO_INCREMENT PRIMARY KEY, entity_type VARCHAR(30) DEFAULT '', entity_id INT NULL,
                title VARCHAR(500) NOT NULL DEFAULT '', description TEXT, assigned_to INT NULL,
                due_date DATETIME NULL, status ENUM('pending','in_progress','done','cancelled') DEFAULT 'pending',
                priority ENUM('low','normal','high','urgent') DEFAULT 'normal', created_by INT NULL,
                completed_at DATETIME NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                INDEX idx_entity (entity_type, entity_id), INDEX idx_assigned (assigned_to), INDEX idx_status (status), INDEX idx_due (due_date)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`);

            await pool.execute(`CREATE TABLE IF NOT EXISTS comments (
                id INT AUTO_INCREMENT PRIMARY KEY, entity_type VARCHAR(30) NOT NULL DEFAULT '', entity_id INT NOT NULL,
                user_id INT NULL, text TEXT NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                INDEX idx_entity (entity_type, entity_id)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`);

            await pool.execute(`CREATE TABLE IF NOT EXISTS lead_stage_history (
                id INT AUTO_INCREMENT PRIMARY KEY, lead_id INT NOT NULL, from_stage VARCHAR(50) DEFAULT '',
                to_stage VARCHAR(50) NOT NULL, user_id INT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                INDEX idx_lead (lead_id)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`);

            await pool.execute(`CREATE TABLE IF NOT EXISTS content_versions (
                id INT AUTO_INCREMENT PRIMARY KEY, entity_type VARCHAR(30) NOT NULL DEFAULT '', entity_id INT NOT NULL,
                lang VARCHAR(10) DEFAULT 'ru', title VARCHAR(500) DEFAULT '', html LONGTEXT, created_by INT NULL,
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                INDEX idx_entity (entity_type, entity_id)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`);

            await pool.execute(`CREATE TABLE IF NOT EXISTS proposals (
                id INT AUTO_INCREMENT PRIMARY KEY, order_id INT NULL, lead_id INT NULL,
                client_name VARCHAR(255) DEFAULT '', client_email VARCHAR(255) DEFAULT '', client_phone VARCHAR(100) DEFAULT '',
                items_json JSON, total DECIMAL(12,2) DEFAULT 0,
                status ENUM('draft','sent','accepted','rejected','expired') DEFAULT 'draft',
                valid_until DATE NULL, notes TEXT, created_by INT NULL,
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                INDEX idx_status (status), INDEX idx_lead (lead_id), INDEX idx_order (order_id)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`);

            await pool.execute(`CREATE TABLE IF NOT EXISTS invoices (
                id INT AUTO_INCREMENT PRIMARY KEY, proposal_id INT NULL, order_id INT NULL,
                client_name VARCHAR(255) DEFAULT '', client_email VARCHAR(255) DEFAULT '',
                items_json JSON, total DECIMAL(12,2) DEFAULT 0,
                status ENUM('draft','sent','paid','overdue','cancelled') DEFAULT 'draft',
                due_date DATE NULL, paid_at DATETIME NULL, notes TEXT, created_by INT NULL,
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                INDEX idx_status (status), INDEX idx_order (order_id)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`);

            await pool.execute(`CREATE TABLE IF NOT EXISTS audit_log (
                id INT AUTO_INCREMENT PRIMARY KEY, user_id INT NULL, username VARCHAR(100) DEFAULT '',
                action VARCHAR(100) NOT NULL DEFAULT '', entity_type VARCHAR(50) DEFAULT '', entity_id INT NULL,
                details TEXT, ip VARCHAR(45) DEFAULT '', created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                INDEX idx_user (user_id), INDEX idx_action (action), INDEX idx_entity (entity_type, entity_id), INDEX idx_created (created_at)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`);

            await pool.execute(`CREATE TABLE IF NOT EXISTS login_attempts (
                id INT AUTO_INCREMENT PRIMARY KEY, username VARCHAR(100) DEFAULT '', ip VARCHAR(45) DEFAULT '',
                success TINYINT DEFAULT 0, created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                INDEX idx_ip (ip), INDEX idx_created (created_at)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`);

            // ── Safe ALTER: add columns if missing ─────
            const safeAddCol = async (table, col, def) => {
                try { await pool.execute(`ALTER TABLE \`${table}\` ADD COLUMN ${col} ${def}`); } catch(e) { /* already exists */ }
            };
            await safeAddCol('leads', 'stage', "VARCHAR(50) DEFAULT 'new' AFTER status");
            await safeAddCol('leads', 'utm_source', "VARCHAR(255) DEFAULT '' AFTER stage");
            await safeAddCol('orders', 'total_price', "DECIMAL(12,2) DEFAULT 0 AFTER status");
            await safeAddCol('leads', 'utm_medium', "VARCHAR(255) DEFAULT '' AFTER utm_source");
            await safeAddCol('leads', 'utm_campaign', "VARCHAR(255) DEFAULT '' AFTER utm_medium");
            await safeAddCol('leads', 'utm_content', "VARCHAR(255) DEFAULT '' AFTER utm_campaign");
            await safeAddCol('leads', 'utm_term', "VARCHAR(255) DEFAULT '' AFTER utm_content");
            await safeAddCol('pages', 'status', "ENUM('draft','published','scheduled','archived') DEFAULT 'published' AFTER html");
            await safeAddCol('pages', 'publish_at', "DATETIME NULL AFTER status");
            await safeAddCol('pages', 'seo_title', "VARCHAR(255) DEFAULT '' AFTER publish_at");
            await safeAddCol('projects', 'deadline_text', "VARCHAR(255) DEFAULT '' AFTER cover_image");
            await safeAddCol('projects', 'duration_text', "VARCHAR(255) DEFAULT '' AFTER deadline_text");
            await safeAddCol('pages', 'seo_description', "TEXT AFTER seo_title");
            await safeAddCol('pages', 'og_title', "VARCHAR(255) DEFAULT '' AFTER seo_description");
            await safeAddCol('pages', 'og_description', "TEXT AFTER og_title");
            await safeAddCol('articles', 'status', "ENUM('draft','published','scheduled','archived') DEFAULT 'published' AFTER cover_image");
            await safeAddCol('articles', 'publish_at', "DATETIME NULL AFTER status");
            await safeAddCol('articles', 'seo_title', "VARCHAR(255) DEFAULT '' AFTER publish_at");
            await safeAddCol('articles', 'seo_description', "TEXT AFTER seo_title");
            await safeAddCol('articles', 'og_title', "VARCHAR(255) DEFAULT '' AFTER seo_description");
            await safeAddCol('articles', 'og_description', "TEXT AFTER og_title");
            await safeAddCol('admin_users', 'totp_secret', "VARCHAR(255) DEFAULT '' AFTER last_login");
            await safeAddCol('admin_users', 'totp_enabled', "TINYINT DEFAULT 0 AFTER totp_secret");
            ...
            try { await pool.execute("ALTER TABLE project_reviews MODIFY COLUMN client_id INT NULL"); } catch(e) { /* already nullable or missing */ }

            // Create default admin if none exists
            const admins = await q('SELECT id FROM admin_users LIMIT 1');
            if (!admins.length) {
                const hash = bcrypt.hashSync('admin123', 10);
                await ex('INSERT INTO admin_users (username, password_hash, display_name, role) VALUES (?,?,?,?)',
    ...
    async function auditLog(req, action, entityType, entityId, details) {
        try {
            ...
            await ex('INSERT INTO audit_log (user_id, username, action, entity_type, entity_id, details, ip) VALUES (?,?,?,?,?,?,?)',
    ...
    async function getAnalyticsOverview(rangeDays) {
        ...
        const [rows] = await pool.execute(
            `SELECT
                 ...
                 SUM(is_calculator_start) as calc_starts,
                 SUM(is_calculator_complete) as calc_completes,
                 SUM(is_lead_submit) as lead_submits
    ...
    async function getAnalyticsBreakdown(rangeDays) {
        const days = Math.min(Math.max(parseInt(rangeDays || '7', 10) || 7, 1), 90);
        const sinceExpr = 'created_at >= DATE_SUB(CURDATE(), INTERVAL ? DAY)';
        const params = [days - 1];
    ...
    router.post('/login', async (req, res) => {
        ...
        if (typeof hooks.isDbReady === 'function' && !hooks.isDbReady()) {
            return res.status(503).json({
                error: 'База данных недоступна. На сервере проверьте MySQL и файл .env: DB_HOST, DB_USER, DB_PASSWORD, DB_NAME. Для Ubuntu часто нужен отдельный пользователь, не root.'
            });
        }
        const ip = req.headers['x-forwarded-for'] || req.socket.remoteAddress || '';
        try {
            // Rate limit: max 10 failed attempts per IP in 15 min
            const [attempts] = await q('SELECT COUNT(*) as c FROM login_attempts WHERE ip = ? AND success = 0 AND created_at > DATE_SUB(NOW(), INTERVAL 15 MINUTE)', [String(ip).slice(0, 45)]);
            if (attempts.c >= 10) return res.status(429).json({ error: 'Слишком много попыток. Подождите 15 минут' });

            const rows = await q('SELECT id, username, password_hash, display_name, role, avatar_url, is_active, totp_enabled FROM admin_users WHERE username = ? LIMIT 1', [String(username).trim()]);
            if (!rows.length) {
                await ex('INSERT INTO login_attempts (username, ip, success) VALUES (?,?,0)', [String(username).slice(0, 100), String(ip).slice(0, 45)]);
            ...
            if (user.totp_enabled) {
                ...
                const [u2] = await q('SELECT totp_secret FROM admin_users WHERE id = ?', [user.id]);
                if (!u2 || !verifyTOTP(u2.totp_secret, String(totp_code))) {
                    return res.status(401).json({ error: 'Неверный код 2FA' });
                }
            }
            await ex('INSERT INTO login_attempts (username, ip, success) VALUES (?,?,1)', [String(username).slice(0, 100), String(ip).slice(0, 45)]);
            req.session.adminId = user.id;
            req.session.adminRole = user.role;
            req.session.adminName = user.display_name;
            await ex('UPDATE admin_users SET last_login = NOW() WHERE id = ?', [user.id]);
    ...
    router.get('/me', requireAdmin, async (req, res) => {
        try {
            const rows = await q('SELECT id, username, display_name, role, avatar_url, totp_enabled FROM admin_users WHERE id = ?', [req.session.adminId]);
    ...
    router.put('/profile', requireAdmin, async (req, res) => {
        ...
        try {
            await ex('UPDATE admin_users SET display_name = ?, avatar_url = ? WHERE id = ?',
    ...
    router.put('/password', requireAdmin, async (req, res) => {
        ...
        try {
            const rows = await q('SELECT password_hash FROM admin_users WHERE id = ?', [req.session.adminId]);
            if (!rows.length) return res.status(401).json({ error: 'Unauthorized' });
            ...
            await ex('UPDATE admin_users SET password_hash = ? WHERE id = ?', [hash, req.session.adminId]);
            res.json({ ok: true });
        } catch (e) {
            res.status(500).json({ error: 'Ошибка сервера' });
        }
    ...
    router.get('/dashboard/stats', requireAdmin, async (req, res) => {
        try {
            ...
            const leadsMonth = await q('SELECT COUNT(*) as c FROM leads WHERE created_at >= ?', [monthAgo]);
            ...
            const visitorsToday = await q('SELECT COUNT(DISTINCT session_id) as c FROM analytics_events WHERE DATE(created_at) = ?', [today]);
            const visitorsWeek = await q('SELECT COUNT(DISTINCT session_id) as c FROM analytics_events WHERE created_at >= ?', [weekAgo]);
            const calcCompleted = await q('SELECT COUNT(*) as c FROM calculator_sessions WHERE completed = 1');
            const projectsCount = await q('SELECT COUNT(*) as c FROM projects WHERE is_published = 1');
            const articlesCount = await q('SELECT COUNT(*) as c FROM articles WHERE is_published = 1');

            // Page views last 7 days
            const dailyViews = await q(
                'SELECT view_date, SUM(views) as total FROM page_views WHERE view_date >= ? GROUP BY view_date ORDER BY view_date',
                [weekAgo]
            );

            // Top pages
            const topPages = await q(
                'SELECT page_url, COUNT(*) as hits FROM analytics_events WHERE created_at >= ? AND event_type = "pageview" GROUP BY page_url ORDER BY hits DESC LIMIT 10',
                [weekAgo]
            );
            ...
            const deviceStats = await q(
                'SELECT device_type, COUNT(DISTINCT session_id) as c FROM analytics_events WHERE created_at >= ? GROUP BY device_type',
                [weekAgo]
            );

            // Visitors by browser
            const browserStats = await q(
                'SELECT browser, COUNT(DISTINCT session_id) as c FROM analytics_events WHERE created_at >= ? GROUP BY browser ORDER BY c DESC LIMIT 8',
                [weekAgo]
            );
            ...
            const geoData = await q(
                'SELECT country, city, COUNT(DISTINCT session_id) as c FROM analytics_events WHERE created_at >= ? AND country IS NOT NULL AND country != "" GROUP BY country, city ORDER BY c DESC LIMIT 30',
                [monthAgo]
            );

            // Visitors by language
            const langStats = await q(
                'SELECT language, COUNT(DISTINCT session_id) as c FROM analytics_events WHERE created_at >= ? AND language != "" GROUP BY language ORDER BY c DESC LIMIT 10',
                [weekAgo]
            );
    ...
    router.get('/leads/:id', requireAdmin, async (req, res) => {
        try {
            const rows = await q('SELECT * FROM leads WHERE id = ?', [req.params.id]);
    ...
    router.put('/leads/:id', requireAdmin, async (req, res) => {
        ...
        try {
            ...
            if (notes !== undefined) { sets.push('notes = ?'); params.push(String(notes).slice(0, 5000)); }
            if (!sets.length) return res.json({ ok: true });
            params.push(req.params.id);
            await ex(`UPDATE leads SET ${sets.join(', ')} WHERE id = ?`, params);
            res.json({ ok: true });
        } catch (e) {
            res.status(500).json({ error: 'Ошибка сервера' });
        }
    ...
    router.delete('/leads/:id', requireRole('admin'), async (req, res) => {
        try {
            await ex('DELETE FROM leads WHERE id = ?', [req.params.id]);
            res.json({ ok: true });
        } catch (e) {
            res.status(500).json({ error: 'Ошибка сервера' });
        }
    ...
    router.get('/orders', requireAdmin, async (req, res) => {
        try {
            ...
            const rows = await q(`SELECT * FROM orders WHERE ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`, [...params, limit, offset]);
            res.json({ ok: true, orders: rows, total: countRow.c, page, limit });
        } catch (e) {
            res.status(500).json({ error: 'Ошибка сервера' });
        }
    });
    ...
    router.post('/orders', requireAdmin, async (req, res) => {
        ...
        try {
            const result = await ex(
                'INSERT INTO orders (lead_id, client_name, client_email, client_phone, service, description, status, total_price, source, source_detail, deadline, notes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)',
    ...
    router.put('/orders/:id', requireAdmin, async (req, res) => {
        ...
        try {
            await ex(
                'UPDATE orders SET client_name=?, client_email=?, client_phone=?, service=?, description=?, status=?, total_price=?, source=?, source_detail=?, deadline=?, notes=? WHERE id=?',
    ...
    router.delete('/orders/:id', requireRole('admin'), async (req, res) => {
        try {
            await ex('DELETE FROM orders WHERE id = ?', [req.params.id]);
            res.json({ ok: true });
        } catch (e) {
            res.status(500).json({ error: 'Ошибка сервера' });
        }
    ...
    router.get('/pages/:slug', requireAdmin, async (req, res) => {
        try {
            ...
            const rows = await q('SELECT * FROM pages WHERE slug = ?', [slug]);
            if (!rows.length) return res.status(404).json({ error: 'Not found' });
            const page = rows[0];
            const locales = await q('SELECT * FROM page_locales WHERE page_id = ?', [page.id]);
    ...
    router.put('/pages/:slug', requireAdmin, async (req, res) => {
        ...
        try {
            const rows = await q('SELECT id FROM pages WHERE slug = ?', [slug]);
            let pageId;
            if (rows.length) {
                pageId = rows[0].id;
                await ex('UPDATE pages SET title = ?, html = ? WHERE id = ?', [title || '', html || '', pageId]);
            } else {
                const r = await ex('INSERT INTO pages (slug, title, html) VALUES (?,?,?)', [slug, title || '', html || '']);
                pageId = r.insertId;
            }
            // Update locales
            if (locales && typeof locales === 'object') {
                for (const [lang, data] of Object.entries(locales)) {
                    ...
                    await ex(
                        'INSERT INTO page_locales (page_id, lang, title, html) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE title=VALUES(title), html=VALUES(html)',
    ...
    router.get('/projects/:id', requireAdmin, async (req, res) => {
        try {
            ...
            if (!rows.length) return res.status(404).json({ error: 'Not found' });
            const locales = await q('SELECT * FROM project_locales WHERE project_id = ?', [rows[0].id]);
    ...
    router.post('/projects', requireRole('admin', 'director'), async (req, res) => {
        ...
        try {
            ...
            const result = await ex('INSERT INTO projects (slug, is_published, cover_image, deadline_text, duration_text) VALUES (?,?,?,?,?)', [s, is_published ? 1 : 0, cover_image || '', deadline_text || '', duration_text || '']);
            const projectId = result.insertId;
            if (locales && typeof locales === 'object') {
                for (const [lang, data] of Object.entries(locales)) {
                    await ex(
                        `INSERT INTO project_locales (project_id, lang, title, excerpt, html, gallery_json, stack_json, stack_front_json, stack_back_json, stack_db_json, stack_deploy_json, stack_android_json, stack_ios_json) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    ...
    router.put('/projects/:id', requireAdmin, async (req, res) => {
        const { slug, is_published, cover_image, deadline_text, duration_text, locales } = req.body;
        try {
            const prevRows = await q('SELECT id, slug FROM projects WHERE id = ? LIMIT 1', [req.params.id]);
            ...
            await ex('UPDATE projects SET slug=?, is_published=?, cover_image=?, deadline_text=?, duration_text=? WHERE id=?',
                [s, is_published ? 1 : 0, cover_image || '', deadline_text || '', duration_text || '', req.params.id]);
            if (locales && typeof locales === 'object') {
                for (const [lang, data] of Object.entries(locales)) {
                    await ex(
                        `INSERT INTO project_locales (project_id, lang, title, excerpt, html, gallery_json, stack_json, stack_front_json, stack_back_json, stack_db_json, stack_deploy_json, stack_android_json, stack_ios_json)
                         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
                         ON DUPLICATE KEY UPDATE title=VALUES(title), excerpt=VALUES(excerpt), html=VALUES(html),
                         gallery_json=VALUES(gallery_json), stack_json=VALUES(stack_json),
                         stack_front_json=VALUES(stack_front_json), stack_back_json=VALUES(stack_back_json),
                         stack_db_json=VALUES(stack_db_json), stack_deploy_json=VALUES(stack_deploy_json),
                         stack_android_json=VALUES(stack_android_json), stack_ios_json=VALUES(stack_ios_json)`,
    ...
    router.delete('/projects/:id', requireRole('admin'), async (req, res) => {
        try {
            const rows = await q('SELECT slug FROM projects WHERE id = ? LIMIT 1', [req.params.id]);
            const prevSlug = rows.length ? (rows[0].slug || '') : '';
            await ex('DELETE FROM projects WHERE id = ?', [req.params.id]);
    ...
    router.get('/articles/:id', requireAdmin, async (req, res) => {
        try {
            const rows = await q('SELECT * FROM articles WHERE id = ?', [req.params.id]);
            if (!rows.length) return res.status(404).json({ error: 'Not found' });
            const locales = await q('SELECT * FROM article_locales WHERE article_id = ?', [rows[0].id]);
    ...
    router.post('/articles', requireRole('admin', 'director'), async (req, res) => {
        ...
        try {
            ...
            const result = await ex('INSERT INTO articles (slug, is_published, cover_image) VALUES (?,?,?)', [s, is_published ? 1 : 0, cover_image || '']);
            const articleId = result.insertId;
            if (locales && typeof locales === 'object') {
                for (const [lang, data] of Object.entries(locales)) {
                    await ex(
                        'INSERT INTO article_locales (article_id, lang, title, excerpt, html, gallery_json) VALUES (?,?,?,?,?,?)',
    ...
    router.put('/articles/:id', requireAdmin, async (req, res) => {
        const { slug, is_published, cover_image, locales } = req.body;
        try {
            const prevRows = await q('SELECT id, slug FROM articles WHERE id = ? LIMIT 1', [req.params.id]);
            if (!prevRows.length) return res.status(404).json({ error: 'Not found' });
            const prevSlug = prevRows[0].slug || '';
            const sourceSlug = slug || firstLocaleTitle(locales) || prevSlug;
            ...
            await ex('UPDATE articles SET slug=?, is_published=?, cover_image=? WHERE id=?',
                [s, is_published ? 1 : 0, cover_image || '', req.params.id]);
            if (locales && typeof locales === 'object') {
                for (const [lang, data] of Object.entries(locales)) {
                    await ex(
                        'INSERT INTO article_locales (article_id, lang, title, excerpt, html, gallery_json) VALUES (?,?,?,?,?,?) ON DUPLICATE KEY UPDATE title=VALUES(title), excerpt=VALUES(excerpt), html=VALUES(html), gallery_json=VALUES(gallery_json)',
    ...
    router.delete('/articles/:id', requireRole('admin'), async (req, res) => {
        try {
            const rows = await q('SELECT slug FROM articles WHERE id = ? LIMIT 1', [req.params.id]);
            const prevSlug = rows.length ? (rows[0].slug || '') : '';
            await ex('DELETE FROM articles WHERE id = ?', [req.params.id]);
    ...
    router.post('/projects/:id/reviews', requireAdmin, async (req, res) => {
        ...
        try {
            const result = await ex(
                'INSERT INTO project_reviews (project_id, client_id, lang, rating, review_text, company_name, logo_url, website, is_published) VALUES (?,?,?,?,?,?,?,?,1)',
                [req.params.id, client_id || null, lang || 'ru', rating || 5, review_text || '', company_name || '', logo_url || '', website || '']
            );
            const [proj] = await q('SELECT slug FROM projects WHERE id = ?', [req.params.id]);
            if (proj) clearPublicCaches([`projectReview:${proj.slug}:`]);
            res.json({ ok: true, id: result.insertId });
    ...
    router.put('/reviews/:id', requireAdmin, async (req, res) => {
        const { company_name, logo_url, website, review_text, rating, lang, client_id } = req.body;
        try {
            const [rev] = await q('SELECT project_id FROM project_reviews WHERE id = ?', [req.params.id]);
            await ex(
                'UPDATE project_reviews SET company_name=?, logo_url=?, website=?, review_text=?, rating=?, lang=?, client_id=? WHERE id=?',
                [company_name || '', logo_url || '', website || '', review_text || '', rating || 5, lang || 'ru', client_id || null, req.params.id]
            );
            if (rev) {
                const [proj] = await q('SELECT slug FROM projects WHERE id = ?', [rev.project_id]);
                if (proj) clearPublicCaches([`projectReview:${proj.slug}:`]);
            }
            res.json({ ok: true });
        } catch (e) { res.status(500).json({ error: 'Ошибка сервера' }); }
    ...
    router.delete('/reviews/:id', requireAdmin, async (req, res) => {
        try {
            const [rev] = await q('SELECT project_id FROM project_reviews WHERE id = ?', [req.params.id]);
            await ex('DELETE FROM project_reviews WHERE id = ?', [req.params.id]);
            if (rev) {
                const [proj] = await q('SELECT slug FROM projects WHERE id = ?', [rev.project_id]);
                if (proj) clearPublicCaches([`projectReview:${proj.slug}:`]);
            }
    ...
    router.get('/clients', requireAdmin, async (req, res) => {
        try {
            const rows = await q(
                `SELECT c.id, c.slug, cp.company_name, cp.logo_url, cp.website, cp.description, cp.address, cp.city, cp.country
                 FROM clients c
                 LEFT JOIN client_profiles cp ON cp.client_id = c.id
    ...
    router.get('/clients/:id', requireAdmin, async (req, res) => {
        try {
            const [row] = await q(
                `SELECT c.id, c.slug, c.email, cp.company_name, cp.logo_url, cp.website, cp.description, cp.address, cp.city, cp.country
                 FROM clients c
                 LEFT JOIN client_profiles cp ON cp.client_id = c.id
                 WHERE c.id = ?`,
                [req.params.id]
    ...
    router.post('/clients', requireAdmin, async (req, res) => {
        const { slug, company_name, logo_url, website, description, address, city, country } = req.body;
        ...
        try {
            ...
            const result = await ex(
                'INSERT INTO clients (slug, email, password_hash, is_active) VALUES (?, ?, ?, 1)',
                [s, fakeEmail, '']
            );
            const clientId = result.insertId;
            await ex(
                'INSERT INTO client_profiles (client_id, company_name, logo_url, website, description, address, city, country) VALUES (?,?,?,?,?,?,?,?)',
                [clientId, company_name || '', logo_url || '', website || '', description || '', address || '', city || '', country || '']
            );
            clearPublicCaches([`client:${s}`]);
            res.json({ ok: true, id: clientId, slug: s });
        } catch (e) {
            if (e.code === 'ER_DUP_ENTRY') return res.status(409).json({ error: 'Клиент с таким slug уже существует' });
            res.status(500).json({ error: 'Ошибка: ' + e.message });
        }
    });
    ...
    router.put('/clients/:id', requireAdmin, async (req, res) => {
        const { slug, company_name, logo_url, website, description, address, city, country } = req.body;
        try {
            const [old] = await q('SELECT slug FROM clients WHERE id = ?', [req.params.id]);
            if (!old) return res.status(404).json({ error: 'not found' });
            const s = String(slug || old.slug).toLowerCase().replace(/[^a-z0-9а-яёa-z\-]/gi, '').replace(/\s+/g, '-').slice(0, 120);
            await ex('UPDATE clients SET slug = ? WHERE id = ?', [s, req.params.id]);
            const [hasProfile] = await q('SELECT id FROM client_profiles WHERE client_id = ?', [req.params.id]);
            if (hasProfile) {
                await ex(
                    'UPDATE client_profiles SET company_name=?, logo_url=?, website=?, description=?, address=?, city=?, country=? WHERE client_id=?',
                    [company_name || '', logo_url || '', website || '', description || '', address || '', city || '', country || '', req.params.id]
                );
            } else {
                await ex(
                    'INSERT INTO client_profiles (client_id, company_name, logo_url, website, description, address, city, country) VALUES (?,?,?,?,?,?,?,?)',
                    [req.params.id, company_name || '', logo_url || '', website || '', description || '', address || '', city || '', country || '']
                );
            }
            clearPublicCaches([`client:${old.slug}`, `client:${s}`]);
            res.json({ ok: true, slug: s });
        } catch (e) {
            if (e.code === 'ER_DUP_ENTRY') return res.status(409).json({ error: 'Клиент с таким slug уже существует' });
            res.status(500).json({ error: 'Ошибка: ' + e.message });
    ...
    router.delete('/clients/:id', requireAdmin, async (req, res) => {
        try {
            const [old] = await q('SELECT slug FROM clients WHERE id = ?', [req.params.id]);
            await ex('DELETE FROM client_profiles WHERE client_id = ?', [req.params.id]);
            await ex('DELETE FROM clients WHERE id = ?', [req.params.id]);
            if (old) clearPublicCaches([`client:${old.slug}`]);
            res.json({ ok: true });
        } catch (e) { res.status(500).json({ error: 'Ошибка сервера' }); }
    });
    ...
    router.get('/settings', requireAdmin, async (req, res) => {
        try {
            ...
            const rows = await q('SELECT setting_key, setting_value FROM settings WHERE setting_key IN (' + keys.map(() => '?').join(',') + ')', keys);
            const settings = {};
    ...
    router.post('/users', requireRole('admin'), async (req, res) => {
        ...
        try {
            ...
            const result = await ex('INSERT INTO admin_users (username, password_hash, display_name, role) VALUES (?,?,?,?)',
                [String(username).trim().slice(0, 100), hash, String(display_name || username).slice(0, 255), role || 'manager']);
            res.json({ ok: true, id: result.insertId });
        } catch (e) {
            if (e.code === 'ER_DUP_ENTRY') return res.status(400).json({ error: 'Такой логин уже существует' });
            res.status(500).json({ error: 'Ошибка сервера' });
        }
    });
    ...
    router.put('/users/:id', requireRole('admin'), async (req, res) => {
        ...
        try {
            ...
            const params = [String(display_name || '').slice(0, 255), role || 'manager', is_active ? 1 : 0];
            if (password && password.length >= 6) {
                sets.push('password_hash = ?');
                params.push(bcrypt.hashSync(String(password), 10));
            }
            params.push(req.params.id);
            await ex(`UPDATE admin_users SET ${sets.join(', ')} WHERE id = ?`, params);
            res.json({ ok: true });
        } catch (e) {
            res.status(500).json({ error: 'Ошибка сервера' });
        }
    ...
    router.delete('/users/:id', requireRole('admin'), async (req, res) => {
        ...
        try {
            await ex('DELETE FROM admin_users WHERE id = ?', [req.params.id]);
            res.json({ ok: true });
        } catch (e) {
            res.status(500).json({ error: 'Ошибка сервера' });
        }
    ...
    router.get('/tasks', requireAdmin, async (req, res) => {
        try {
            ...
            if (req.query.status && req.query.status !== 'all') { where += ' AND t.status = ?'; params.push(req.query.status); }
            if (req.query.entity_type) { where += ' AND t.entity_type = ?'; params.push(req.query.entity_type); }
            if (req.query.entity_id) { where += ' AND t.entity_id = ?'; params.push(req.query.entity_id); }
            ...
            const rows = await q(`SELECT t.*, au.display_name as assignee_name, au2.display_name as creator_name FROM tasks t LEFT JOIN admin_users au ON t.assigned_to=au.id LEFT JOIN admin_users au2 ON t.created_by=au2.id WHERE ${where} ORDER BY FIELD(t.priority,'urgent','high','normal','low'), t.due_date ASC LIMIT ? OFFSET ?`, [...params, limit, offset]);
            res.json({ ok: true, tasks: rows, total: countRow.c, page, limit });
    ...
    router.post('/tasks', requireAdmin, async (req, res) => {
        ...
        try {
            const result = await ex('INSERT INTO tasks (entity_type, entity_id, title, description, assigned_to, due_date, priority, created_by) VALUES (?,?,?,?,?,?,?,?)',
            ...
            await auditLog(req, 'create_task', 'task', result.insertId, title);
            res.json({ ok: true, id: result.insertId });
        } catch (e) { res.status(500).json({ error: e.message }); }
    });
    ...
    router.put('/tasks/:id', requireAdmin, async (req, res) => {
        ...
        try {
            ...
            await ex(`UPDATE tasks SET title=?, description=?, assigned_to=?, due_date=?, status=?, priority=?, completed_at=${completedAt} WHERE id=?`,
    ...
    router.delete('/tasks/:id', requireAdmin, async (req, res) => {
        try {
            await ex('DELETE FROM tasks WHERE id = ?', [req.params.id]);
    ...
    router.get('/comments', requireAdmin, async (req, res) => {
        ...
        try {
            const rows = await q('SELECT c.*, au.display_name as user_name FROM comments c LEFT JOIN admin_users au ON c.user_id=au.id WHERE c.entity_type=? AND c.entity_id=? ORDER BY c.created_at DESC', [entity_type, entity_id]);
    ...
    router.post('/comments', requireAdmin, async (req, res) => {
        ...
        try {
            const result = await ex('INSERT INTO comments (entity_type, entity_id, user_id, text) VALUES (?,?,?,?)',
    ...
    router.delete('/comments/:id', requireRole('admin'), async (req, res) => {
        try {
            await ex('DELETE FROM comments WHERE id = ?', [req.params.id]);
            res.json({ ok: true });
        } catch (e) { res.status(500).json({ error: e.message }); }
    });
    ...
    router.put('/leads/:id/stage', requireAdmin, async (req, res) => {
        ...
        try {
            const [lead] = await q('SELECT stage FROM leads WHERE id = ?', [req.params.id]);
            const fromStage = lead ? (lead.stage || 'new') : 'new';
            await ex('UPDATE leads SET stage = ? WHERE id = ?', [stage, req.params.id]);
            await ex('INSERT INTO lead_stage_history (lead_id, from_stage, to_stage, user_id) VALUES (?,?,?,?)',
    ...
    router.get('/leads/:id/history', requireAdmin, async (req, res) => {
        try {
            const rows = await q('SELECT h.*, au.display_name as user_name FROM lead_stage_history h LEFT JOIN admin_users au ON h.user_id=au.id WHERE h.lead_id=? ORDER BY h.created_at DESC', [req.params.id]);
    ...
    router.get('/proposals', requireAdmin, async (req, res) => {
        try {
            ...
            const rows = await q('SELECT p.*, au.display_name as creator_name FROM proposals p LEFT JOIN admin_users au ON p.created_by=au.id ORDER BY p.created_at DESC LIMIT ? OFFSET ?', [limit, offset]);
            res.json({ ok: true, proposals: rows, total: countRow.c, page, limit });
        } catch (e) { res.status(500).json({ error: e.message }); }
    });

    router.get('/proposals/:id', requireAdmin, async (req, res) => {
        try {
            const rows = await q('SELECT * FROM proposals WHERE id = ?', [req.params.id]);
            if (!rows.length) return res.status(404).json({ error: 'Not found' });
    ...
    router.post('/proposals', requireAdmin, async (req, res) => {
        ...
        try {
            const result = await ex('INSERT INTO proposals (order_id, lead_id, client_name, client_email, client_phone, items_json, total, valid_until, notes, created_by) VALUES (?,?,?,?,?,?,?,?,?,?)',
            ...
            await auditLog(req, 'create_proposal', 'proposal', result.insertId, client_name);
            res.json({ ok: true, id: result.insertId });
        } catch (e) { res.status(500).json({ error: e.message }); }
    });
    ...
    router.put('/proposals/:id', requireAdmin, async (req, res) => {
        ...
        try {
            await ex('UPDATE proposals SET client_name=?, client_email=?, client_phone=?, items_json=?, total=?, status=?, valid_until=?, notes=? WHERE id=?',
    ...
    router.delete('/proposals/:id', requireRole('admin'), async (req, res) => {
        try {
            await ex('DELETE FROM proposals WHERE id = ?', [req.params.id]);
    ...
    router.get('/invoices', requireAdmin, async (req, res) => {
        try {
            ...
            const rows = await q(`SELECT i.*, au.display_name as creator_name FROM invoices i LEFT JOIN admin_users au ON i.created_by=au.id WHERE ${where} ORDER BY i.created_at DESC LIMIT ? OFFSET ?`, [...params, limit, offset]);
            res.json({ ok: true, invoices: rows, total: countRow.c, page, limit });
        } catch (e) { res.status(500).json({ error: e.message }); }
    });

    router.get('/invoices/:id', requireAdmin, async (req, res) => {
        try {
            const rows = await q('SELECT * FROM invoices WHERE id = ?', [req.params.id]);
            if (!rows.length) return res.status(404).json({ error: 'Not found' });
    ...
    router.post('/invoices', requireAdmin, async (req, res) => {
        ...
        try {
            const result = await ex('INSERT INTO invoices (proposal_id, order_id, client_name, client_email, items_json, total, due_date, notes, created_by) VALUES (?,?,?,?,?,?,?,?,?)',
            ...
            await auditLog(req, 'create_invoice', 'invoice', result.insertId, client_name);
            res.json({ ok: true, id: result.insertId });
        } catch (e) { res.status(500).json({ error: e.message }); }
    });
    ...
    router.put('/invoices/:id', requireAdmin, async (req, res) => {
        ...
        try {
            await ex('UPDATE invoices SET client_name=?, client_email=?, items_json=?, total=?, status=?, due_date=?, paid_at=?, notes=? WHERE id=?',
    ...
    router.delete('/invoices/:id', requireRole('admin'), async (req, res) => {
        try {
            await ex('DELETE FROM invoices WHERE id = ?', [req.params.id]);
    ...
    router.get('/revenue', requireAdmin, async (req, res) => {
        try {
            ...
            const monthly = await q(`SELECT DATE_FORMAT(created_at, '%Y-%m') as month, SUM(total_price) as revenue, COUNT(*) as count
            ...
            res.json({
                ok: true,
                revenue: {
                    total: Number(totalRev.s), active: Number(activeRev.s),
                    invoiced: Number(invoicedTotal.s), paid: Number(invoicePaid.s),
                    overdue: Number(invoiceOverdue.s), avgCheck: Number(avgCheck.a),
    ...
    router.get('/versions', requireAdmin, async (req, res) => {
        ...
        try {
            const rows = await q('SELECT v.id, v.lang, v.title, v.created_by, v.created_at, au.display_name as user_name FROM content_versions v LEFT JOIN admin_users au ON v.created_by=au.id WHERE v.entity_type=? AND v.entity_id=? ORDER BY v.created_at DESC LIMIT 50', [entity_type, entity_id]);
    ...
    router.get('/versions/:id', requireAdmin, async (req, res) => {
        try {
            const rows = await q('SELECT * FROM content_versions WHERE id = ?', [req.params.id]);
            if (!rows.length) return res.status(404).json({ error: 'Not found' });
    ...
    router.post('/versions', requireAdmin, async (req, res) => {
        ...
        try {
            const result = await ex('INSERT INTO content_versions (entity_type, entity_id, lang, title, html, created_by) VALUES (?,?,?,?,?,?)',
    ...
    router.get('/audit', requireRole('admin'), async (req, res) => {
        try {
            ...
            const offset = (page - 1) * limit;
            let where = '1=1'; const params = [];
            if (req.query.user_id) { where += ' AND user_id = ?'; params.push(req.query.user_id); }
            ...
            const rows = await q(`SELECT * FROM audit_log WHERE ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`, [...params, limit, offset]);
            res.json({ ok: true, logs: rows, total: countRow.c, page, limit });
        } catch (e) { res.status(500).json({ error: e.message }); }
    });
    ...
    router.post('/2fa/setup', requireAdmin, async (req, res) => {
        try {
            ...
            await ex('UPDATE admin_users SET totp_secret = ? WHERE id = ?', [secret, req.session.adminId]);
            const rows = await q('SELECT username FROM admin_users WHERE id = ?', [req.session.adminId]);
    ...
    router.post('/2fa/enable', requireAdmin, async (req, res) => {
        const { code } = req.body;
        try {
            const [user] = await q('SELECT totp_secret FROM admin_users WHERE id = ?', [req.session.adminId]);
            if (!user || !user.totp_secret) return res.status(400).json({ error: 'Сначала настройте 2FA' });
            if (!verifyTOTP(user.totp_secret, String(code))) return res.status(400).json({ error: 'Неверный код' });
            await ex('UPDATE admin_users SET totp_enabled = 1 WHERE id = ?', [req.session.adminId]);
    ...
    router.post('/2fa/disable', requireAdmin, async (req, res) => {
        ...
        try {
            ...
            if (user.totp_secret && !verifyTOTP(user.totp_secret, String(code))) return res.status(400).json({ error: 'Неверный код' });
            await ex('UPDATE admin_users SET totp_enabled = 0, totp_secret = "" WHERE id = ?', [req.session.adminId]);
